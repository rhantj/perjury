...
export const ROLES: readonly Role[] = [
  ...
  },
  {
    ...
    hanja: '巡査',
    side: 'citizen',
    power: '한 명을 지목한다 — 그가 다음에 선언하는 반증이 참인지 통보받는다.',
  ...
  },
  {
    ...
    side: 'citizen',
    /*
     * 원래 문구는 「반증 요구를 1회 거부한다」였다.
     * 침묵도 이의제기 대상이 아니라, 문구만 보고는 무엇이 이득인지 보이지 않았다.
     * 갈리는 지점은 위증 기록이다 — 침묵은 남고(순사·사진사·신문기자에 걸린다) 거부는 남지 않는다.
     */
    power: '반증 요구를 1회 거부한다 — 위증 기록이 남지 않는다.',
  ...
  },
  {
    ...
    hanja: '寫眞師',
    side: 'citizen',
    power: '한 명을 촬영한다 — 그가 다음 라운드부터 처음 선언할 때 위증하면 즉시 발각된다.',
...
/**
 * 발동이 없이 «판 내내» 켜져 있는 직업인가.
 *
 * 전화교환수는 회선이 하나 늘어난 채로 판이 시작되므로(결정 007) `usePower`를 한 번도
 * 부르지 않는다. 그래서 「壹回 — 한 번 남았다」로 그리면 쓸 것이 남았다는 거짓말이 된다.
 *
 * **판정을 여기 두는 이유는 뱃지를 그리는 화면이 둘이기 때문이다** —
 * 착석 전 브리핑(Briefing)과 착석 후 내 명패(MyPlate). 각자 판정하면 갈라진다.
 * 실제로 갈라져서 명패만 고쳐지고 브리핑은 「壹回」인 채로 남아 있었다.
 */
export function isAlwaysOn(role: Role): boolean {
  return role.effect === 'eavesdrop'
}
