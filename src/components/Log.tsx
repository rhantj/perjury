...
interface Line {
  ...
  text: string
  /**
   * LLM이 쓴 대사. **여기서는 text를 대체하지 않고 나란히 둔다 — 좌석과 다르다.**
   *
   * 좌석은 대사가 있으면 그것만 보여준다(Table.tsx). 칸이 좁아 두 줄이 겹치는 데다,
   * 반증한 좌석에는 카드 그림이 따로 떠서 무엇으로 반증했는지가 남기 때문이다.
   * 기록에는 그 그림이 없고, 폭도 넉넉하다. 그리고 여기는 **되짚어 보는 곳**이라
   * 「무엇으로 반증했나」가 추리의 근거다 — 대사가 카드 이름을 말하지 않으면 그게 사라진다.
   *
   * 없으면 null이다(사람·규칙 기반 판단자·폴백).
   */
  quote: string | null
  mine: boolean
}
