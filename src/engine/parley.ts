...
/**
 * 한 판에 걸 수 있는 밀담 횟수 (decisions/009, 룰 개편 §3-2).
 *
 * 라운드마다 문이 열리므로 제한이 없으면 24회차 = 24번이고, 그러면 «언제 쓸까»가 사라진다.
 * **3회차당 1회**로 묶어 밀담을 아껴 쓰는 자원으로 남긴다.
 *
 * 회차 상한과 «같이» 정해야 하는 값이다. 08-06에 상한만 8→24로 올리고 이 값을 3에 두어
 * 8회차당 1회가 됐는데, 그 상태를 200판 돌려 보니 24회차를 다 쓰고도
 * **한 판도 혼자 풀리지 않았다**(직접 지운 카드 평균 5.76/12장, 남은 조합 약 27가지).
 * 좁히는 것이 아니라 아예 못 풀게 만드는 값이었다.
 *
 * **6으로 고친 것도 모자랐다.** 밀담이 메워야 하는 격차가 평균 6.27장(최악 10장)인데
 * 6회로는 한 번도 빗나가지 않아야 겨우 닿아서, 이론상 도달 가능한 판이 56.8%였다.
 * 8회면 96.0%다. 10회면 100%지만 여유가 4회로 벌어져 위증에 속아도 다시 물으면 되는
 * 헐거운 판이 된다 — 「모자라지만 못 풀 정도는 아닌」 선이 8이다.
 *
 * 둘이 갈라지지 않게 `parley.test.ts`가 비율을 묶어 둔다.
 */
export const PARLEY_LIMIT = 8
 ...
 * (결정 007), 라운드로 세면 그 판이 예산 6회를 넘겨 열두 번까지 말하게 된다.
...
export function parley(
  ...
  /*
   * 받는 쪽을 막는 것은 비용이 아니라 **밸런스** 결정이다(결정 011 §4·§6).
   *
   * 손패를 지키려는 것이 아니다 — 밀담은 여기서 카드를 한 장도 열지 않는다.
   * 막는 이유는 **탈락자를 지목해 원할 때, 값 없이, 반복해서 여는 유일한 통로**라서다.
   * 탈락자는 승패에서 빠져 거짓말할 이유가 없으니 사실상 정직한 정보원이 되고,
   * 그러면 조기 고발이 「틀려도 정보원이 하나 생긴다」가 되어 실패의 값이 사라진다.
   *
   * 호출 절약이 근거가 아니다 — 호출은 이 함수보다 먼저 나간다(store의 askParley).
   */
   ...
   * 아래 라운드당 1회는 «한 라운드 안에서 두 번»을, 이쪽은 «판 전체에서 일곱 번»을 막는다.
   *
