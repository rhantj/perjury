...
describe('parley — 판당 횟수 제한 (decisions/009)', () => {
   ...
   *
   * 3회차당 1회라는 비율이 근거다(결정 009의 08-07 두 번째 추가). 상한을 8에서 24로 올린
   * 08-06에 밀담을 같이 올리지 않아 8회차당 1회가 됐고, 그때 「정보가 마른다」가 실측으로
   * 나왔다 (24회차를 다 돌고도 200판 전부 혼자서는 못 풀었다).
   *
   * 4:1(6회)로 한 번 고쳤지만 그것도 모자랐다 — 정답을 특정하려면 평균 6.27장을 더
   * 지워야 하는데 6회로는 딱 맞아떨어져야만 닿아서, 이론상 도달 가능한 판이 56.8%였다.
   * 3:1(8회)이면 96.0%가 된다.
   *
   * **회차만 바꾸면 여기가 터진다. 그게 이 테스트의 목적이다** — 두 값은 따로 정할 수 없다.
   */
  it('한도는 회차 상한에 3:1로 묶인다', () => {
    expect(
      PARLEY_LIMIT * 3,
      '밀담은 3회차당 1회다. 회차를 바꿨다면 결정 009를 먼저 고치고 여기로 온다',
    ).toBe(DEFAULT_ROUNDS)
  })
