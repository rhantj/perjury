...
/**
 * 배경음은 계속 깔리므로 효과음보다 낮게 둔다. 대사를 읽는 화면이라 소리가 앞서면 방해가 된다.
 *
 * 다만 «낮게»가 «안 들리게»가 되어 있었다(0.32 / 0.55).
 *
 * **여기 값만 봐서는 안 된다.** 실제 음량은 이 값 × engine의 MASTER_TRIM이고,
 * 트림이 0.95에서 2.2로 올라갔으므로 배경음은 숫자가 0.52 → 0.42로 «내려갔는데»
 * 실제로는 더 커진다(0.52×1.35 = 0.70 → 0.42×2.2 = 0.92). 배경음만 따로 내린 이유는
 * 지속음이라 효과음처럼 잠깐 눌리고 마는 게 아니라, 리미터를 계속 밀고 있으면
 * 효과음이 날 때마다 배경음이 같이 주저앉아 «출렁»이기 때문이다.
 */
