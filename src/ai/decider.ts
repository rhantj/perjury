...
export interface Spoken<T> {
  ...
  /**
   * 제안 대신 **조기 고발**을 하겠다는 의사. 없으면 평소대로 제안한다 (룰 개편 §2-6).
   *
   * `power`와 같은 이유로 판단 호출에 얹는다 — 전용 메서드를 두면 라운드당 호출이 늘고
   * 그만큼 실패 노출면이 커진다(결정 006). 제안자는 chooseSuggestion을 정확히 한 번
   * 부르므로, 이 필드 하나로 **라운드마다 한 명씩 돌아가며** 「지금 외칠까」를 판단하게 된다.
   *
   * **제안 판단에서만 읽는다.** 다른 kind에 실어도 flow가 보지 않는다.
   *
   * 판정은 하지 않는다 — 맞고 틀림은 engine의 accuseEarly가 정하고, 틀리면 외친 사람이
   * 탈락한다. 여기 있는 것은 «외치겠다»는 의사뿐이다(작업 규칙 2).
   */
  readonly accuse?: Suggestion | null
}
