...
| **Lighthouse** (배포본) | 성능 **94** · 접근성 **100** · 권장사항 96 · SEO 100 | LCP 1.8초 · TBT 10ms · **CLS 0** — 레이아웃을 움직이는 연출을 안 쓴 결과다 |
...
Lighthouse가 잡아낸 것 중 **안 고친 것 두 가지**도 적는다.

- **favicon 404** — `<link rel="icon">`이 없어 브라우저가 도메인 루트의 `/favicon.ico`를 찾다 404를 낸다.
  게임 동작에는 영향이 없지만 콘솔에 에러가 하나 남는다.
- **음소거 버튼의 「Label in Name」 위반** — 눈에 보이는 글자는 `音 소리`인데
  `aria-label`은 `소리 끄기`라 둘이 안 맞는다(WCAG 2.5.3). 음성 제어로 "音"이라 말하면 눌리지 않는다.
  Lighthouse 접근성 점수에는 가중치 0이라 100점에 안 잡히지만, 실제 결함은 결함이다.
