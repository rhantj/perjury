...
function rulesBlock(label: Label): string {
  return [
    ...
    /*
     * 비공개 반증(룰 개편 1-B). 이 두 줄이 없으면 모델이 «남의 반증 카드를 나도 봤다»고
     * 믿고 추론한다 — 실제 기록에는 「반증(카드 비공개)」만 실리므로 근거 없는 단정이 된다.
     */
    '- **반증에 쓴 카드는 제안자에게만 보인다.** 나머지 좌석에는 「반증했다」는 사실만 전해진다.',
    '- 반면 **반증하지 못했다는 것은 전원이 안다.** 그자가 제안된 세 장을 하나도 갖고 있지 않다는 뜻이다.',
    '  이것이 모두에게 열리는 유일한 단서이므로 여기서부터 좁혀 나가라.',
    '- **위증: 손에 없는 카드로도 반증할 수 있다. 규칙 위반이 아니다.**',
    '  다만 누군가 이의를 제기해 발각되면 손패가 공개된다.',
    '- 이의제기: 남의 반증이 거짓이라고 판단되면 지목한다. 틀리면 내 카드가 공개된다.',
    '  증명하려면 그자가 냈다는 카드를 내가 쥐고 있어야 한다. 카드가 보이지 않으면 증명할 수 없다.',
    ...
    /*
     * 덫 제안. 이 세 줄이 없으면 위증이 사실상 처벌되지 않는다 — 반증 카드를 보는 사람이
