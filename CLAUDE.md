...
- 기본 모델 `claude-sonnet-5` (2026-08-03 실측 후 결정, `docs/decisions/003-운영-llm-모델.md`).
  워커 `LLM_MODEL` 환경변수에 있다. 변경은 대표 결정 사항이므로 임의로 바꾸지 않는다.
- 반증 판단·투표는 **스키마 강제 구조화 출력**으로 받는다. 자유 텍스트 파싱으로 게임 진행을 결정하지 않는다.
  Anthropic에서는 `output_config.format.type = "json_schema"`다. **강제 `tool_choice`는 쓰지 않는다** —
  그쪽은 확장 사고와 병용하면 400이 난다.
...
- Claude 5 계열은 thinking이 기본 ON이고 `max_tokens`가 thinking+응답 합산 상한이다. `temperature`/`top_p`는 미지원 —
...
### 실제 플레이 확인은 배포 링크에서 한다

**동작 확인은 https://rhantj.github.io/perjury/ 에서 한다.** 로컬 개발 서버가 아니다.

심사자가 보는 것이 배포본이고, 로컬에서만 되는 것은 되는 게 아니기 때문이다.
실제로 로컬과 배포본은 갈라진다 — `base: '/perjury/'` 경로, 정적 import가 아닌 자산,
`import.meta.env.DEV`로 갈리는 값(프록시 주소), CORS 허용 오리진이 전부 그렇다.

- `npm run dev`는 코드를 짜는 동안 쓰는 것이지 **완료를 판단하는 근거가 아니다.**
- 배포 링크에서 확인하려면 `main`에 올라가 있어야 한다. 그래서 **푸시 전 빌드 통과가 전제**다.
- **워커도 함께 배포돼 있어야 한다.** 배포본 프론트는 `*.workers.dev`를 부르므로,
  워커가 옛 버전이면 프론트만 새것이어도 폴백만 보게 된다. `npm run worker:deploy`.
- 배포 직후 곧바로 확인하면 전파 지연으로 옛 화면이나 엣지 오류가 보인다.
  **코드를 고치러 들어가기 전에 1분 기다렸다 다시 본다.**
- 로컬에서만 되는 확인(프록시 실패 주입, 예산 캡 초과 등)은 로컬로 하되,
  **완료 보고에는 배포 링크에서 본 것만 적는다.**
